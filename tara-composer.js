(() => {
  ...
  document.querySelectorAll('.mode-pill').forEach(btn => btn.addEventListener('click', () => {
    ...
    mode = btn.dataset.mode || 'deep';
    setStatus(`${btn.textContent.trim()} mode selected.`);
    input.focus();
  }));
  ...
  async function companySuggestions(q) {
    try {
      ...
      const rows = Array.isArray(d.companies) ? d.companies : Array.isArray(d.results) ? d.results : [];
      autocomplete.innerHTML = '';
      rows.slice(0, 6).forEach(c => {
        const b = document.createElement('button');
        b.type = 'button'; b.className = 'suggestion';
        const symbol = c.nseSymbol || c.bseSymbol || c.symbol || '';
        const exchange = c.exchanges || c.exchange || '';
        b.innerHTML = `<strong>${esc(c.name || symbol)}</strong><span>${esc(symbol)}${exchange ? ` · ${esc(exchange)}` : ''}${c.isin ? ` · ${esc(c.isin)}` : ''}</span>`;
        b.addEventListener('click', () => { input.value = symbol ? `${symbol} ` : `${c.name || ''} `; autocomplete.classList.remove('show'); resize(); input.focus(); });
  ...
  fileInput?.addEventListener('change', () => {
    ...
    if (file) setStatus(`Attached: ${file.name}. File analysis will activate when the document-analysis provider is connected.`);
  });
  ...
  voiceBtn?.addEventListener('click', () => {
    ...
    recognition.onresult = event => {
      input.value = Array.from(event.results).map(x => x[0].transcript).join(''); resize();
    };
    recognition.onerror = () => setStatus('Voice input could not be completed.', 'error');
  ...
  form?.addEventListener('submit', async e => {
    ...
    setStatus('Tara is analyzing your request with the native intelligence layer…');
    try {
      const r = await fetch('/api/ai/chat', {
        method: 'POST', headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
        body: JSON.stringify({ message: `[${mode}] ${message}`, language: document.documentElement.lang || 'en' })
      });
      const d = await r.json().catch(() => ({}));
      if (!r.ok) throw new Error(d.error || 'Tara AI is temporarily unavailable.');
      resultText.textContent = esc(d.reply || d.message || 'Verified evidence is not available for this request yet.');
      const native = d.analysis || d.nativeAnalysis;
      if (native?.decision === 'insufficient_verified_evidence') {
        setStatus('Verified data is unavailable for a reliable conclusion.', 'error');
      } else {
        setStatus('Tara analysis ready.', 'ok');
      }
    } catch (err) {
      resultText.textContent = esc(err.message || 'Tara AI is temporarily unavailable.');
      setStatus('Tara could not complete the request.', 'error');
    } finally { send.disabled = false; }
  });

  document.addEventListener('click', e => {
    if (!e.target.closest('.tara-composer-box')) autocomplete.classList.remove('show');
  });
  resize();
})();