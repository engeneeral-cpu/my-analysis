...
function searchRank(c, q) {
  ...
  const fields = [[c.nseSymbol, 100], [c.bseCode, 100], [c.bseSymbol, 100], [c.isin, 100], [c.name, 80]]; let best = 0;
...
function registerCompanyRoutes(app) {
  ...
  const searchHandler = async (req, res) => { try { const universe = await getUniverse(); const q = String(req.query.q || '').trim(), exchange = String(req.query.exchange || 'all').trim().toLowerCase(), limit = Math.min(Math.max(Number(req.query.limit) || 10, 1), 500); const filtered = exchange === 'nse' ? universe.rows.filter(c => c.exchanges.includes('NSE')) : exchange === 'bse' ? universe.rows.filter(c => c.exchanges.includes('BSE')) : universe.rows; const matches = filtered.map(c => ({ ...c, searchRelevance: searchRank(c, q) })).filter(c => !q || c.searchRelevance > 0).sort((a, b) => b.searchRelevance - a.searchRelevance || a.name.localeCompare(b.name)).slice(0, limit); res.json({ success: true, query: q, totalMatches: matches.length, source: universe.sources, updatedAt: new Date(universe.at).toISOString(), coverage: coverage(universe), companies: matches, results: matches }); } catch (error) { console.error('[Tara Company Search]', error.message); res.status(503).json({ success: false, error: 'Company search is temporarily unavailable.' }); } };
  app.get('/api/companies', searchHandler); app.get('/api/companies/search', searchHandler);
  app.post('/api/tara/analyze', async (req, res) => { try { const symbol = String(req.body?.symbol || '').trim().toUpperCase().replace(/[^A-Z0-9._-]/g, ''); if (!symbol) return res.status(400).json({ success: false, error: 'Valid NSE/BSE symbol is required' }); const data = req.body?.marketData || {}; const required = ['currentPrice', 'rsi', 'sma20', 'sma50', 'pe', 'debtToEquity']; const missing = required.filter(k => data[k] === undefined || data[k] === null); if (missing.length) return res.json({ success: true, data: { status: 'EVIDENCE_GATEKEEPER_BLOCKED', symbol, verdict: 'INSUFFICIENT_VERIFIED_EVIDENCE', conclusion: null, blockedReasons: missing.map(k => `Missing ${k}`), nextRequired: missing, disclaimer: 'Tara AI does not generate market conclusions without verified evidence.' } }); const validation = validateMetrics(data); if (!validation.isValid) return res.json({ success: true, data: { status: 'EVIDENCE_GATEKEEPER_BLOCKED', symbol, verdict: 'INSUFFICIENT_VERIFIED_EVIDENCE', conclusion: null, blockedReasons: validation.errors, disclaimer: 'Tara AI does not generate market conclusions from unverified or anomalous data.' } }); const analysis = calculateMultiFactorScore(data); return res.json({ success: true, data: { status: 'ANALYSIS_COMPLETE', engine: 'Tara AI Native Intelligence Engine', version: '2.1.0-native-evidence', symbol, taraScore: `${analysis.score}/100`, verdict: analysis.verdict, strategicStance: analysis.stance, factorBreakdown: analysis.factors, dataSource: { name: data.dataSource.name, retrievedAt: data.dataSource.retrievedAt, licenseType: data.dataSource.licenseType || null }, disclaimer: 'Educational market research only; not SEBI registered investment advice.' } }); } catch (error) { return res.status(500).json({ success: false, error: 'Tara analysis could not be completed.' }); } });
}
module.exports = { registerCompanyRoutes, getUniverse, coverage };
