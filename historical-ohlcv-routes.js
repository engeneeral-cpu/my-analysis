...
function authHeaders() { const key = String(process.env.MARKET_HISTORY_API_KEY || '').trim(); if (!key) return {}; return String(process.env.MARKET_HISTORY_AUTH_MODE || 'bearer').toLowerCase() === 'x-api-key' ? { 'X-API-Key': key } : { Authorization: `Bearer ${key}` }; }
...
function uniqueOrdered(rows, interval = '1d') { const map = new Map(); for (const row of rows) { const c = normalizeCandle(row, interval); if (c) map.set(c.time, c); } return [...map.values()].sort((a, b) => a.time.localeCompare(b.time)); }
...
function providerConfig() { return { name: process.env.MARKET_HISTORY_PROVIDER_NAME || null, type: String(process.env.MARKET_HISTORY_PROVIDER_TYPE || '').toLowerCase(), apiUrl: String(process.env.MARKET_HISTORY_API_URL || '').trim(), apiKey: String(process.env.MARKET_HISTORY_API_KEY || '').trim() }; }
...
async function fetchConfiguredProvider(company, range, interval) {
  ...
  const raw = await fetchJson(url.toString(), String(process.env.MARKET_HISTORY_AUTH_MODE || 'bearer').toLowerCase() === 'query' ? {} : authHeaders()); return Array.isArray(raw) ? raw : (raw?.data?.candles || raw?.data || raw?.candles || raw?.results || raw?.history || []);
}
...
function metadata(verified, notice = null) { const cfg = providerConfig(); return { source: verified ? (cfg.name || null) : null, provider_type: cfg.type || null, verified, exchange_holidays_excluded: true, notice }; }
async function historyHandler(req, res) {
  ...
  try {
    ...
    const candles = uniqueOrdered(await fetchRows(company, range, interval), interval);
    if (!candles.length) return res.json({ success: true, symbol, range, interval, candles: [], metadata: metadata(false, 'Historical exchange data pending ingestion') });
