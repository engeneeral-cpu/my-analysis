const ENGINE_VERSION = '1.0.0-native';
...
function calculateTaraScore(data={}){
  ...
  const rsi=num(data.rsi), price=num(data.currentPrice ?? data.price), sma50=num(data.sma50), pe=num(data.pe);
  ...
  return {score,factors,confidence:factors.length?Math.min(95,45+factors.length*15):0};
}
...
function analyze(input={}){
  ...
  ['price','previousClose','volume','revenue','profit','eps','debt','cashFlow'].forEach(k=>add(k,input[k]));
  const technical=calculateTaraScore(input), completeness=Math.round(verified.length/8*100);
