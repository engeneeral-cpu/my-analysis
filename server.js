...
const apiLimiter = rateLimit({ windowMs: 60 * 1000, limit: 120, standardHeaders: 'draft-8', legacyHeaders: false, message: { success: false, error: 'Too many requests. Please try again later.' } });
app.use('/api', apiLimiter);
const authLimiter = rateLimit({ windowMs: 15 * 60 * 1000, limit: 20, standardHeaders: 'draft-8', legacyHeaders: false, message: { success: false, error: 'Too many authentication attempts. Please try again later.' } });
...
app.post('/api/analyze', (req, res) => {
  ...
  res.json({ success: true, aiName: 'Tara AI', found: false, text: `Tara AI is ready to research "${query}". Source-backed company intelligence is served through the company API.` });
});

app.use('/api', (req, res) => res.status(404).json({ success: false, error: 'Tara AI API endpoint not found.' }));
app.use((err, req, res, next) => {
  if (err?.message === 'CORS origin denied') return res.status(403).json({ success: false, error: 'Origin not allowed' });
  console.error('[Tara Security] Request error:', err?.message || 'Unknown error');
