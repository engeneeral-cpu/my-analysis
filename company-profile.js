...
async function loadHistory(range='1y'){
 ...
 if(!body)return;
 body.innerHTML='<div class="history-loading">Loading verified historical market data…</div>';
 ...
 try{
  ...
  if(!r.ok||!d.success)throw new Error(d?.error||'Historical data unavailable');
  renderHistory(d,range);
 }catch(e){
  status.textContent='Data unavailable';
  body.innerHTML=`<div class="history-empty"><strong>Historical data is not connected yet.</strong><p>${esc(e.message)}</p><small>No fabricated prices are displayed. Connect an authorized/licensed EOD provider to populate Daily through All Time.</small></div>`;
 }
}
